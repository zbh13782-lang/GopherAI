...
func GetMessagesBySessionID(sessionID string) ([]model.Message, error) {
	...
	err := mysql.DB.Where("session_id = ?", sessionID).Order("create_at asc").Find(&msgs).Error
	return msgs, err
}
...
func GetMessagesBySessionIDs(sessionIDs []string) ([]model.Message, error) {
	...
	err := mysql.DB.Where("session_id IN ?", sessionIDs).Order("create_at asc").Find(&msgs).Error
	return msgs, err
}

func GetAllMessages() ([]model.Message, error) {
	var msgs []model.Message
	err := mysql.DB.Order("create_at asc").Find(&msgs).Error
	return msgs, err
}
